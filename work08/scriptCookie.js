...
function onClickBtn3(element) {
    ...
    //     }
    // });
    let divElement = createDivElement(getCookies('test2'));
    document.querySelector('#work1').appendChild(divElement);
...
function onClickBtn4(element) {
    ...
    let divElement = createDivElement(getLocalstorage('test3'));
    document.querySelector('#work1').appendChild(divElement);
...
window.addEventListener('load', (e) => {
    ...
    document.body.style.opacity = count;
    let newCount = Number(count) - 0.1;
    if (newCount < 0) {
        localStorage.setItem('count', 1);
    } else {
